...
vec3 sumDirLights(vec3 diffuseColor, vec3 envSample, float specMask, vec3 viewDir, vec3 nrm)
{
    ...
    for (int i = 0; i < NUM_DIR_LIGHTS; ++i)
    {
        ...
        // Specular
        float specAmt = specular(light.direction, viewDir, nrm, 16.0);
...
void main()
{
    ...
    // Diffuse
    vec3 diffuseColor = vec3(WATER_R, WATER_G, WATER_B);
    ...
    // Alpha
    float alpha = 1- length(cameraPos - fragWorldPos) * 0.2;

    
    outCol = vec4(finalColor + ambient, alpha);
}